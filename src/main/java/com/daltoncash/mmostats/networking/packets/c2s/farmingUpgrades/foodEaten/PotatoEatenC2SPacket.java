...
public class PotatoEatenC2SPacket {

	public PotatoEatenC2SPacket() {

	}

	public PotatoEatenC2SPacket(FriendlyByteBuf buf) {
	...
	public boolean handle(Supplier<NetworkEvent.Context> supplier) {
		...
		context.enqueueWork(() -> {
			...
			player.getCapability(PotatoEatenProvider.SUM).ifPresent(total -> {
				total.addSum(1);
				ModMessages.sendToPlayer(new PotatoEatenDataSyncS2CPacket(total.getSum()), player);
			});
		});
		return true;
	}
