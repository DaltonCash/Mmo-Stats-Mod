...
public class GainMiningExpFromMultiblockC2SPacket {

	public GainMiningExpFromMultiblockC2SPacket() {

	}

	public GainMiningExpFromMultiblockC2SPacket(FriendlyByteBuf buf) {
	...
	public boolean handle(Supplier<NetworkEvent.Context> supplier) {
		...
		context.enqueueWork(() -> {
			...
			player.getCapability(PlayerMiningExpProvider.PLAYER_MINING_EXP).ifPresent(miningExp -> {
				miningExp.addMiningExp(Math.round(SkillForgeEvents.expToAddMultiblock * ModStats.getMiningModifier()));
