...
public class BeefEatenC2SPacket {

	public BeefEatenC2SPacket() {

	}

	public BeefEatenC2SPacket(FriendlyByteBuf buf) {
	...
	public boolean handle(Supplier<NetworkEvent.Context> supplier) {
		...
		context.enqueueWork(() -> {
			...
			player.getCapability(BeefEatenProvider.SUM).ifPresent(total -> {
				total.addSum(1);
				ModMessages.sendToPlayer(new BeefEatenDataSyncS2CPacket(total.getSum()), player);
			});
		});
		return true;
	}
