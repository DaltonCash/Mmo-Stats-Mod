...
public class FishEatenC2SPacket {

	public FishEatenC2SPacket() {

	}

	public FishEatenC2SPacket(FriendlyByteBuf buf) {
	...
	public boolean handle(Supplier<NetworkEvent.Context> supplier) {
		...
		context.enqueueWork(() -> {
			...
			player.getCapability(FishEatenProvider.SUM).ifPresent(total -> {
				total.addSum(1);
				ModMessages.sendToPlayer(new FishEatenDataSyncS2CPacket(total.getSum()), player);
			});
		});
		return true;
	}
