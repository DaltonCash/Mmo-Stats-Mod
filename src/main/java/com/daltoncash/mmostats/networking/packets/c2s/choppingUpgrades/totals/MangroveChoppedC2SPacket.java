...
public class MangroveChoppedC2SPacket {

	public MangroveChoppedC2SPacket() {

	}

	public MangroveChoppedC2SPacket(FriendlyByteBuf buf) {
	...
	public boolean handle(Supplier<NetworkEvent.Context> supplier) {
		...
		context.enqueueWork(() -> {
			...
			player.getCapability(MangroveChoppedProvider.TOTAL).ifPresent(total -> {
				total.addSum(1);
				ModMessages.sendToPlayer(new MangroveChoppedDataSyncS2CPacket(total.getSum()), player);
			});
		});
		return true;
	}
