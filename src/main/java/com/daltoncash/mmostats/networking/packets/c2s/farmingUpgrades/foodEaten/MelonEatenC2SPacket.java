...
public class MelonEatenC2SPacket {

	public MelonEatenC2SPacket() {

	}

	public MelonEatenC2SPacket(FriendlyByteBuf buf) {
	...
	public boolean handle(Supplier<NetworkEvent.Context> supplier) {
		...
		context.enqueueWork(() -> {
			...
			player.getCapability(MelonEatenProvider.SUM).ifPresent(total -> {
				total.addSum(1);
				ModMessages.sendToPlayer(new MelonEatenDataSyncS2CPacket(total.getSum()), player);
			});
		});
		return true;
	}
