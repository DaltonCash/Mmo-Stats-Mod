...
public class CrimsonStemChoppedC2SPacket {

	public CrimsonStemChoppedC2SPacket() {

	}

	public CrimsonStemChoppedC2SPacket(FriendlyByteBuf buf) {
	...
	public boolean handle(Supplier<NetworkEvent.Context> supplier) {
		...
		context.enqueueWork(() -> {
			...
			player.getCapability(CrimsonStemChoppedProvider.TOTAL).ifPresent(total -> {
				total.addSum(1);
				ModMessages.sendToPlayer(new CrimsonStemChoppedDataSyncS2CPacket(total.getSum()), player);
			});
		});
		return true;
	}
