...
public class GainSwordsExpC2SPacket {

	public GainSwordsExpC2SPacket() {

	}

	public GainSwordsExpC2SPacket(FriendlyByteBuf buf) {
	...
	public boolean handle(Supplier<NetworkEvent.Context> supplier) {
		...
		context.enqueueWork(() -> {
			...
			player.getCapability(PlayerSwordsExpProvider.PLAYER_SWORDS_EXP).ifPresent(swordsExp -> {
				swordsExp.addSwordsExp(ClientForgeEvents.expToAdd);
