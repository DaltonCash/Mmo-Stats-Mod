...
public class GainChoppingExpFromMultiblockC2SPacket {
	
	public GainChoppingExpFromMultiblockC2SPacket() {

	}
	public GainChoppingExpFromMultiblockC2SPacket(FriendlyByteBuf buf) {
	...
	public boolean handle(Supplier<NetworkEvent.Context> supplier) {
		...
		context.enqueueWork(() -> {
			...
			player.getCapability(PlayerChoppingExpProvider.PLAYER_CHOPPING_EXP).ifPresent(choppingExp -> {
				choppingExp.addChoppingExp(Math.round(SkillForgeEvents.expToAddMultiblock * ModStats.getChoppingModifier()));
